...
app.post('/', async (req, res) => {
  ...
  async function callAI({ url, model, text, textForImage, apiKey, jsonMode = false, max_tokens }) {
    const payload = {
      model,
    };
    if (max_tokens) {
  ...
  const prompt = await callAI({
    ...
    text: `${text}를 바탕으로 축구에 관련하여 어울리는 AI 이미지 생성을 위한 200자 이내의 영어 프롬프트를 작성해줘`,
  ...
  const prompt2 = await callAI({
    ...
    apiKey: GROQ_API_KEY,
    model: GROQ_LLAMA_MODEL,
    text: `${text}를 바탕으로 축구에 연관된 추천에 어울리는 설명 생성을 위한 200자 이내의 한글 프롬프트를 작성해줘`,
  }).then((res) => res.choices[0].message.content);
  console.log(prompt2);
  // 3-2. 그거에서 프롬프트만 추출
  // mixtral-8x7b-32768 (groq)
